# [PeerPilot](https://ooo-a2.github.io/IT-Product-Development-course/)
[![GitHub Pages with Jekyll](https://github.com/OOO-A2/IT-Product-Development-course/actions/workflows/pages.yml/badge.svg)](https://ooo-a2.github.io/IT-Product-Development-course/)

<p align="center">
  <img src="docs/assets/peerpilot-logo.svg" alt="PeerPilot – Peer review · Grades · Transparency" width="320" />
</p>

A self-hosted platform to automate peer reviews, track grades, and streamline course coordination — built to replace spreadsheets with structured clarity.

For more details follow the [Strategic Plan document](https://ooo-a2.github.io/IT-Product-Development-course/plan.html)

## How to Run

To run the website follow the instructions in [frontend/peer-pilot folder](https://github.com/OOO-A2/IT-Product-Development-course/tree/main/frontend/peer-pilot). To setup the server navigate to [backend folder](https://github.com/OOO-A2/IT-Product-Development-course/tree/main/backend/peer-pilot)

## Documentation

The documentation is available online via [the link](https://ooo-a2.github.io/IT-Product-Development-course/)
