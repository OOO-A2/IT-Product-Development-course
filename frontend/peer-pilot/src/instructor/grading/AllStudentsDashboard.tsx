...
export default function AllStudentsDashboard() {
    ...
    const getSumColor = (sum: number, maxSum: number = 500) => {
        const percentage = (sum / maxSum) * 100;
        if (percentage >= 90) return 'bg-green-100 text-green-800 border border-green-300';
        if (percentage >= 80) return 'bg-blue-100 text-blue-800 border border-blue-300';
        if (percentage >= 70) return 'bg-yellow-100 text-yellow-800 border border-yellow-300';
        if (percentage >= 60) return 'bg-orange-100 text-orange-800 border border-orange-300';
                                ...
                                return (
                                    <tr key={student.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        {sprints.flatMap(sprint => {
                                            ...
                                            return [
                                                ...
                                                <td key={`${student.id}-${sprint}-team`} className="px-2 py-4 whitespace-nowrap text-center border-l border-gray-100">
                                                    <span className={`inline-flex items-center px-2 py-1 rounded text-sm font-medium ${getGradeColor(teamGrade, 400)}`}>
                                                        {teamGrade || '-'}
                                                    </span>
                                                </td>,
                                                ...
                                                <td key={`${student.id}-${sprint}-extra`} className="px-2 py-4 whitespace-nowrap text-center border-l border-gray-100">
                                                    {isEditing ? (
                                                        ...
                                                        <button
                                                            ...
                                                            className={`inline-flex items-center px-2 py-1 rounded text-sm font-medium ${getGradeColor(extraGrade)}`}
                                                        >
                                                            {extraGrade || '-'}
                                                        </button>
                                                    )}
                                                ...
                                                <td key={`${student.id}-${sprint}-sum`} className="px-2 py-4 whitespace-nowrap text-center border-l border-gray-200 bg-gray-50">
                                                    <span className={`inline-flex items-center px-2 py-1 rounded text-sm font-semibold ${getSumColor(sprintSum, 500)}`}>
                                                        {sprintSum || '-'}
                                                    </span>
                                                </td>
                                            ];
                                        ...
                                        <td className="px-2 py-4 whitespace-nowrap text-center border-l border-gray-200 bg-gray-100">
                                            <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-bold ${getSumColor(totalSum, 2000)}`}>
                                                {totalSum || '-'}
                                            </span>
                                        </td>
                                    </tr>
